...
#undef STRING_ADD
      ...
      if (handle_opt) {
         ...
         WARN( options, "Missing \"device_handle\" option\n", "" );
         return 1;
      }
   ...
   if (handle_opt) {
      ...
      switch (handle_opt->value_type) {
         ...
         case oyVAL_STRUCT:
            ...
            break;
         default:
            WARN( device, "Option \"device_handle\" is of a wrong type", "");
            break;
      }
      ...
      if (device_handle.get() && device_handle->good())
         ...
         WARN( device, "Unable to open raw image.","");
         return 1;
      }
   } else
      return 1;
